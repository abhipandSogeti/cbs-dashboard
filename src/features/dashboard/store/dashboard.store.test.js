...
describe('useDashboardStore', () => {
    it('starts with population as the active view', () => {
        const { result } = renderHook(() => useDashboardStore());
        expect(result.current.activeView).toBe('population');
    });
    ...
    it('updates table state for a specific view without affecting others', () => {
        const { result } = renderHook(() => useDashboardStore());
        act(() => result.current.setTableState('labour', { globalFilter: 'Amsterdam' }));
